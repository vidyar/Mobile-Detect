...
class Mobile_Detect
{
    ...
    /**
     * This is the current context in which the
     * detection is executed.
     * It gets filled step by step, depends on
     * the methods the user triggers.
     *
     * @since 2.7.7
     * @var array
     */
    ...
    * This is the fastest mobile check possible.
    * Used inside isMobile() method.
    *
    * @return bool
    */
     ...
     * in which we execute the mobile detection rules.
     *
     */
    public function setContextProperty( $name, $value )
    {
    ...
    /**
    * Check if the device is mobile (including tablets).
    ...
    */
    ...
    {
        // Check the HTTP headers for any signs.
        if ( $this->checkHttpHeadersForMobile() ) {
            ...
            // 1. Check phones.
            foreach( $this->getPhoneDevices() as $key => $prop ) {
                // Any match here implies the device is mobile.
                if( $this->match($prop['match'], $this->userAgent) ) {
                    $this->setContextProperty('isMobile', true);
                    return true;
                }
            }

            // 2. Check tablets.
            foreach( $this->getTabletDevices() as $key => $prop ) {
                // Any match here implies the device is mobile.
                if( $this->match($prop['match'], $this->userAgent) ) {
                    $this->setContextProperty('isMobile', true);
                    $this->setContextProperty('isTablet', true);
                    return true;
                }
            }

            // 3. Check operating systems.
            foreach( $this->getOperatingSystems() as $key => $childs ) {
                if( is_array($childs) ) {
                    foreach( $childs as $childKey => $childProp ) {
                        // Check only operating systems that trigger mobile.
                        if( $childProp['isMobile'] ) {
                            if( $this->match($childProp['match'], $this->userAgent) ) {
                                $this->setContextProperty('operatingSystemFamily', $key);
                                $this->setContextProperty('operatingSystem', $childKey);
                                return true;
                            }
                        }
                    }
            ...
            // 4. Check browsers.
            foreach( $this->getBrowsers() as $key => $childs ){
                if( is_array($childs) ) {
                    foreach( $childs as $childKey => $childProp ) {
                        // Check only browsers that trigger mobile.
                        if( $childProp['isMobile'] ) {
                            if( $this->match($childProp['match'], $this->userAgent) ) {
                                $this->setContextProperty('browserFamily', $key);
                                $this->setContextProperty('browser', $childKey);
                                return true;
                            }
                        }
                    }
